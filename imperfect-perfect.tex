...
\begin{defn}
Let $\calR_{K, A}(1)$ denote the object of $\PhiGamma_{K, A}$ corresponding to the cyclotomic character in $\Rep_A(G_K)$ via Theorem~\ref{T:phi gamma embedding}.
%There is then a canonical residue map
%\[
%\Res: H^2_{\varphi, \Gamma}(\calR_{K,A}(1)) \to A
%\]
%which when $[K: \QQ_p] < \infty$ corresponds to the residue map from Tate local duality.
%
For $M \in \PhiGamma_{K,A}$, define the \emph{Cartier dual}
\[
M^* = \Hom_{\calR_{K,A}}(M, \calR_{K,A}(1)) \in \PhiGamma_{K,A};
\]
for $M$ corresponding to $V \in \Rep_A(G_K)$, $M^*$ corresponds to the Cartier dual of $V$.
\end{defn}
...
\end{prop}
\begin{proof}
It suffices to treat the case $K = F$.
The case $F = \QQ_p$ is treated in \cite[Proposition~3.3.2(1)]{kpx}; the same argument applies to general $F$.
\end{proof}

\begin{cor}
For any $M \in \PhiGamma_{K,A}$, the $A$-module $M^{\varphi=1}$ is finitely generated.
\end{cor}
\begin{proof}
It suffices to treat the case $K = F$.
In this case, as in \cite[Notation~2.3.13]{kpx}, we define a
nondegenerate pairing $\{-, - \}: M \times M^* \to A$ such that for all $x \in M$, $y \in M^*$,
\begin{align}
...
\{\varphi(x), y\} &= \{x, \psi(y)\}.
\end{align}
The claim will follow from Proposition~\ref{P:psi finite} upon showing that the pairing $\{-,-\}$ induces an isomorphism
\[
M^{\varphi=1} \cong \Hom_A(M^*/(\psi-1), A).
\]
To begin with, the nondegeneracy of the pairing defines an injection $M \to \Hom_A(M^*, A)$;
