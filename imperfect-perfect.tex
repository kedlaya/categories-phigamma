...
\date{\textit{unstable draft}; version of August 16, 2015}
...
note that $\iota$ obeys a cocycle condition expressing the compatibility of the $G$-action with composition in the group $G$. Note that the description of $\iota$ involves only finitely elements of the ring $\tilde{\calR}^{[s,r]}_{L',A}$, so it may be realized over some subfield of $L'$ which is the completion of an algebraic extension of $L$ of at most countable degree; we may thus assume that $L'$ itself has this form. From the proof of \cite[Theorem~9.2.15]{kedlaya-liu1}, we see that $\tilde{\calR}^{[s,r]}_{L',A}$ splits in the category of Banach modules over $\tilde{\calR}^{[s,r]}_{L,A}$ (initially for $A = \QQ_p$, but then immediately in general);
tensoring $M'$ with this splitting yields a finite projective $\tilde{\calR}^{[s,r]}_{L,A}$-module $M$ which inherits (from $\iota$) an isomorphism
$M \otimes_{\tilde{\calR}^{[s,r]}_{L,A}} \tilde{\calR}^{[s,r]}_{L',A} \cong M'$
of modules over $\tilde{\calR}^{[s,r]}_{L',A}$. Moreover, the cocycle condition on $\iota$ ensures that this isomorphism is compatible with the $G$-actions on both sides.
...
for which see (a), see \cite[Corollary~5.2.4]{kedlaya-liu1}. 
\end{proof}
...
\begin{defn}
For $M \in \PhiMod_{L,A}$ and $n \in \ZZ$, define the twist $M(n)$ to have the same underlying module, with the action of $\varphi$ multiplied by $p^{-n}$.
\end{defn}

\begin{lemma} \label{L:enough phi-invariants}
For $M \in \PhiMod_{L,A}$, there exists $n_0 \in \ZZ$ such that for all $n \geq n_0$,
$M(n)$ is generated as a $\tilde{\calR}_{L,A}$-module by $M(n)^{\varphi}$.
\end{lemma}
\begin{proof}
The proof of \cite[Proposition~6.2.4]{kedlaya-liu1} adapts directly.
\end{proof}
...
Let $\VB_{L,A}$ be the category of vector bundles (or more precisely, quasicoherent sheaves which are locally finite free) on the scheme $\Proj(P_{L,A})$.

The map $\theta$ defines a closed immersion $\Spec(L \widehat{\otimes}_{\QQ_p} A) \to X_{L,A}$; let $Z_{L,A}$ be the resulting closed subscheme of $X_{L,A}$ and let $U_{L,A}$ be its complement.
...
For $\calF \in \VB_{L,A}$, for each $f \in P_{L,A}$ homogeneous of positive degree,
the sections of $\calF$ over the distinguished open affine subspace defined by $f$
form a finite projective module over $(\tilde{\calR}_{L,A}[f^{-1}])^{\varphi}$; by base extension, we obtain a finite projective module over $\tilde{\calR}_{L,A}[f^{-1}]$ equipped with a semilinear $\varphi$-action. By \cite[Lemma~6.3.7]{kedlaya-liu1}, the possible values of $f$ generate the unit ideal in $\tilde{\calR}_{L,A}$, so we may glue to obtain an object of $\PhiMod_{L,A}$.

In the other direction, for $M \in \PhiMod_{L,A}$, we may then view
...
However, by Lemma~\ref{L:units}, this means that $x$ has a well-defined valuation in $v(A)$, which is the same as the valuation of $\varphi(x)$; we must then have $n=0$, contradiction.
\end{proof}
...
The categories $\PhiGamma_{K,A}$ and $\PhiGammatilde_{K,A}$ are canonically equivalent.
\end{theorem}
\begin{proof}
We first describe the functor. 
Choose a coherent sequence $\zeta_p, \zeta_{p^2}, \dots$ of $p$-power roots of unity.
As in Remark~\ref{R:not connected2},
we may project $\calR_{K,A}$ onto a direct summand corresponding to a connected component of $K \otimes_{F} F(\mu_{p^\infty})$. The resulting ring then admits a unique $\varphi$-equivariant embedding into $\tilde{\calR}_{K,A}$
taking $1+\pi$ to $[\epsilon]$ for $\epsilon \in (1,\zeta_p,\zeta_{p^2},\dots) \in L^{\flat}$. Base extension along this embedding gives rise to the desired functor.
For the proof that it is an equivalence, we may imitate the proof in the case $A = \QQ_p$ given in \cite{kedlaya-liu2}.
\end{proof}
...
K.S. Kedlaya, Reified valuations and adic spaces,
arXiv.
