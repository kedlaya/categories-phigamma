...
This has the crucial advantage of making ``evaluation at a point'' make enough sense so that Fontaine's \'etale--de Rham construction factors through the $(\varphi, \Gamma)$-module. Among the various applications of this refinement, we single out one which will be relevant later in our story: one can use $(\varphi, \Gamma)$-modules in this sense to give explicit formulas in Iwasawa theory, such as for the Bloch--Kato exponential formula and the Perrin-Riou reciprocity map \cite{berger-explicit}.
...
the latter holds in turn if $F/K$ is a not totally unramified Galois extension whose Galois group is a $p$-adic Lie group, by a theorem of Sen \cite{sen-lie}.
\end{remark}
...
\end{lemma}
\begin{proof}
Suppose first that $F/K$ is deeply ramified. It is clear that the value group of $L$ must be $p$-divisible (so in particular $L$ is not discretely valued) 
and the residue field of $L$ must be perfect. To prove that $L$ is perfectoid, it then suffices to choose some $\lambda \in \frakm_F$ such that $\lambda^p$ divides $p$ in $\frako_F$, then check that
the Frobenius map from $\frako_F/(\lambda)$ to $\frako_F/(\lambda^p)$ is surjective.
To this end, choose $x \in \frako_F$. Since the set of $p$-th powers in $\frako_F$ is closed,
...
\]
The trace of such an element is $\sum_{i=0}^{p-1} pz_i (-y_0/\mu)^i$, which is necessarily divisible by $p/\mu^{p-1}$; this contradicts the assumption that $F/K$ is deeply ramified.
We thus conclude that $L$ is perfectoid.
...
If we relax the hypothesis on $K$ to allow it to be a more general local field, the vanishing of $H^i_{\varphi, \Gamma}(M)$ for $i>2$ remains valid, but the finite generation of $H^i_{\varphi,\Gamma}$ for $i=0,1,2$ can fail.
\end{remark}
