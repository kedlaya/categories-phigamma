...
\DeclareMathOperator{\BP}{\mathbf{BP}}
...
\date{\textit{unstable draft}; version of August 14, 2015}
...
\begin{defn}
Let $\calR_{K,A} \in \PhiGamma_{F,A}$ be the object of rank $[F:K]$ corresponding to $\Ind^{G_F}_{G_K} \rho_{\triv}$ via Theorem~\ref{T:phi gamma embedding}. The canonical isomorphism $\rho_{\triv} \otimes \rho_{\triv} \cong \rho_{\triv}$
then corresponds to an associative morphism $\mu_K: \calR_{K,A} \otimes \calR_{K,A} \to \calR_{K,A}$; this gives $\calR_{K,A}$ the structure of a finite flat $\calR_{F,A}$-algebra equipped with continuous actions of $\varphi$ and $\Gamma$.
...
We begin with some basic facts from the theory of \emph{perfectoid fields}. This theory subsumes the earlier theory of \emph{norm fields} on which the classical theory of $(\varphi, \Gamma)$-modules is built; see \cite{kedlaya-new-phigamma} for more historical discussion.
...
Suppose that $K = F$ and $L$ is the completion of $K(\mu_{p^\infty})$.
Then 
\begin{align*}
...
Let $L$ be a perfectoid field and define the multiplicative monoids
\[
...
\item[(d)]
The functor $L' \mapsto L^{\prime \flat}$ defines an equivalence of categories between finite extensions of $L$ and $L^{\flat}$. Consequently, there is a canonical isomorphism $G_L \cong G_{L^{\flat}}$.
\end{enumerate}
\end{theorem}
\begin{proof}
...
\textbf{$\varphi$-modules}

\end{defn}

\begin{lemma}
For any perfectoid field $L$, we have $\tilde{\calR}_L^{\varphi} = \QQ_p$.
\end{lemma}
\begin{proof}
...
\end{proof}
...
Following up on Remark~\ref{R:not connected}, we may now see that if $A$ is connected, then the connected components of $\calR_{K,A}$ correspond canonically to the connected components of $K \otimes_F F(\mu_{p^\infty})$; the group $\Gamma$ acting on $\calR_{K,A}$ may be canonically identified with $\Gal(F(\mu_{p^\infty})/F)$; and the stabilizer in $\Gamma$ of any connected component of $K \otimes_F F(\mu_{p^\infty})$ is
the open subgroup $\Gamma_K = \Gal(K(\mu_{p^\infty})/K)$ of $\Gamma$. In the prior literature (excluding \cite{kedlaya-new-phigamma}), it is customary to replace $\calR_{K,A}$ with a connected component and then replace $\Gamma$ with $\Gamma_K$; this gives an equivalent category of $(\varphi, \Gamma)$-modules. See \cite[Remark~2.2.12]{kedlaya-new-phigamma} for further discussion.
\end{remark}
