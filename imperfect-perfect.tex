...
\]
Let $\PhiMod_{L,A}$ be the category of finite projective $\tilde{\calR}_{L,A}$-modules equipped with semilinear $\varphi$-actions. Note that unlike for $(\varphi, \Gamma)$-modules, the action of $\varphi$ on an object of $\PhiMod_{L,A}$ is bijective, so we also have an induced action of $\varphi^{-1}$.
\end{defn}
...
\end{lemma}
\begin{proof}
Again using \cite[Lemma~2.2.9(b)]{kedlaya-liu1}, this reduces to the case $A = \QQ_p$, for which see \cite[Corollary~5.2.4]{kedlaya-liu1}. 
\end{proof}
...
\item[(b)]
the category of finite projective $\tilde{\calR}^r$-modules equipped with isomorphisms 
\[
\varphi^* M  \cong 
...
In the other direction, for $M \in \PhiMod_{L,A}$ and $n \in \ZZ$, we may multiply the action of $\varphi$ on $M$ by $p^{-n}$ to obtain a new object $M(n) \in \PhiMod_{L,A}$. We may then view
...
As in the proof of \cite[Theorem~6.3.12]{kedlaya-liu1}, this reduces to showing that
for $n$ sufficiently large, $M(n)$ is generated as a $\tilde{\calR}_{L,A}$-module by
$M(n)^{\varphi}$; this follows by imitating the proof of \cite[Proposition~6.2.4]{kedlaya-liu1}.
\end{proof}
...
\begin{theorem}
The categories $\PhiGamma_{K,A}, \PhiGammatilde_{K,A}, \BPair_{K,A}, \VB_{K,A}$ are canonically equivalent.
\end{theorem}
\begin{proof}
By Theorem~\ref{T:perfect equivalence}, we need only exhibit the equivalence between
$\PhiGamma_{K,A}$ and $\PhiGammatilde_{K,A}$. This will be given by the base extension functor $M \mapsto M \otimes_{\calR_{K,A}} \tilde{\calR}_{L,A}$.
\end{proof}
