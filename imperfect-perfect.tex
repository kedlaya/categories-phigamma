...
\begin{defn}
For $M \in \PhiGamma_{K,A}$, let $C_{\psi, \Gamma}(M)$ denote the total complex associated to the double complex
\[
...
Theorem~\ref{T:tilde no tilde}. Then there is a canonical morphism of complexes
\[
C_{\psi,\Gamma}(M \boxtimes \Dfm_{K}) \to C_{\psi}(M)
\]
which is a quasi-isomorphism; consequently, the complexes
\[
C_{\psi}(M), C_{\psi,\Gamma}(M \boxtimes \Dfm_{K}), 
C_{\varphi,\Gamma}(M \boxtimes \Dfm_{K}),
C_{\varphi,\Gamma}(\tilde{M} \boxtimes \Dfm_{K})
\]
are all quasi-isomorphic.
\end{theorem}
\begin{proof}
...
\begin{cor}
For $V \in \Rep_A(G_K)$ corresponding to $M \in \PhiGamma_{K,A}$ via 
...
\[
H^i_{\Iw}(G_K, V) \widehat{\otimes}_{\ZZ_p\llbracket \Gamma_K \rrbracket} \Lambda_K \cong H^i_\psi(M) \cong H^i_{\phi, \Gamma}(\tilde{M} \boxtimes \Dfm_K)
\]
